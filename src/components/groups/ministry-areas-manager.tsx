...
import { AlertTriangle, CalendarDays, MoreVertical, TrendingUp, Trash2, UsersRound } from "lucide-react";
...
export default function MinistryAreasManager({
	...
	const filteredAreas = ministryAreas.filter(area => {
		const matchesSearch = !searchTerm ||
			area.name.toLowerCase().includes(searchTerm.toLowerCase());
		const matchesMentor = !mentorFilter ||
			area.mentorId === mentorFilter;
			...
			{filteredAreas.length > 0 ? (
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{filteredAreas.map((area) => {
						...
						const dias = daysSince(area.lastMeetingDate);
						...
						return (
							...
							<Link
								key={area.id}
								href={`/groups/ministry-areas/${area.id}/admin`}
									...
									<CardContent className="space-y-3 pb-4">
										...
										<div className="flex items-center gap-2 pt-1 border-t border-border/40">
											<span className={cn(
												...
												attendanceBadgeClass(area.avgAttendancePct)
											)}>
												<TrendingUp className="h-3 w-3" />
												{area.avgAttendancePct != null ? `${area.avgAttendancePct}%` : "—"}
											</span>
											<span className={cn(
												"inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium",
												diasWarning
				...
				<div className="text-center py-10">
					...
					<h2 className="text-xl font-semibold text-muted-foreground">
						{searchTerm || mentorFilter ? "No se encontraron Áreas" : "No hay Áreas Ministeriales disponibles"}
					</h2>
					<p className="text-muted-foreground mt-2">
						{searchTerm || mentorFilter
							? "Intentá con otros filtros de búsqueda."
							: "Agregá una nueva área ministerial para comenzar."
						}
					</p>
				</div>
			)}
