...
export async function getAllMembers(
	...
	const members = (await findDocuments<MemberDocument>(
		MEMBERS_COLLECTION,
		query,
		{
...
export async function getAllMembersNonPaginated(): Promise<Member[]> {
	return findDocuments<MemberDocument>(
		MEMBERS_COLLECTION,
		{},
		{ sort: { firstName: 1, lastName: 1 } },
	) as Promise<Member[]>;
...
export async function getMemberById(id: string): Promise<Member | null> {
	...
	if (!oid) return null;
	return findOneDocument<MemberDocument>(MEMBERS_COLLECTION, {
		_id: oid,
...
export async function addMember(
	...
	const allGdis = (await findDocuments<GDIDocument>(GDIS_COLLECTION)) as any[];
	const allMinistryAreas = (await findDocuments<MinistryAreaDocument>(
	...
	const calculatedRoles = calculateMemberRoles(
		tempMemberForRoleCalc,
		allGdis,
		allMinistryAreas,
	);
	...
	const insertedMember = (await insertOneDocument<MemberDocument>(
		MEMBERS_COLLECTION,
		memberDoc,
	)) as Member;
	await addMemberToAssignments(insertedMember as Member);
...
export async function updateMember(
	...
	return updateOneDocument<MemberDocument>(
		MEMBERS_COLLECTION,
		{ _id: oid },
		{ $set: finalUpdates },
	);
...
export async function deleteMember(memberId: string): Promise<Member | null> {
	...
	await withTransaction(async (session) => {
		...
		if (
			memberToDelete.assignedAreaIds &&
			memberToDelete.assignedAreaIds.length > 0
		) {
...
export async function bulkRecalculateAndUpdateRoles(
	memberIds: string[],
): Promise<number> {
	...
	const membersToUpdate = await findDocuments<MemberDocument>(
		MEMBERS_COLLECTION,
		{ _id: { $in: memberOids } },
	);
	...
	for (const member of membersToUpdate) {
		...
		const rolesChanged =
			JSON.stringify(calculatedRoles) !== JSON.stringify(member.roles);
