...
import type { GDI, GDIDocument } from "@/lib/types";
...
export async function getAllGdis(): Promise<GDI[]> {
	return findDocuments<GDIDocument>(
		GDIS_COLLECTION,
		{},
		{ sort: { name: 1 } },
	) as Promise<GDI[]>;
...
export async function getGdiById(id: string): Promise<GDI | null> {
	...
	if (!oid) return null;
	return findOneDocument<GDIDocument>(GDIS_COLLECTION, {
		_id: oid,
	}) as Promise<GDI | null>;
}
...
export async function addGdi(gdiData: Omit<GDI, "id">): Promise<GDI> {
	...
	const insertedGdi = (await insertOneDocument<GDIDocument>(
		GDIS_COLLECTION,
		newGdiDoc,
	)) as GDI;
...
export async function updateGdiAndSyncMembers(
	...
	return withTransaction(async (session) => {
		...
		if (updates.guideId !== undefined)
			finalUpdateData.guideId = toObjectId(updates.guideId);
