...
export default function RoleTypesSettingsPage() {
	...
	const loadRoleTypes = async () => {
		...
		try {
			...
			toast({
				title: "Error",
				description: "No se pudieron cargar las etiquetas eclesiásticas",
				variant: "destructive",
			});
		} finally {
			setIsLoading(false);
	...
	const handleCreateRoleType = () => {
		...
		startTransition(async () => {
			try {
				const created = await roleTypesService.create(newRoleTypeName.trim());
				setRoleTypes((prev) => [...prev, created]);
				setNewRoleTypeName("");
				setIsCreateDialogOpen(false);
				toast({ title: "Éxito", description: `Etiqueta "${created.name}" creada correctamente` });
			} catch {
	...
	const handleUpdateRoleType = (roleType: RoleType) => {
		...
		startTransition(async () => {
			try {
				const updated = await roleTypesService.update(roleType.id, trimmed);
				setRoleTypes((prev) => prev.map((rt) => (rt.id === updated.id ? updated : rt)));
				cancelEdit();
				toast({ title: "Éxito", description: `Etiqueta renombrada a "${updated.name}"` });
			} catch {
	...
	const handleDeleteRoleType = (roleType: RoleType) => {
		startTransition(async () => {
			try {
				await roleTypesService.delete(roleType.id);
				setRoleTypes((prev) => prev.filter((rt) => rt.id !== roleType.id));
				toast({ title: "Éxito", description: `Etiqueta "${roleType.name}" eliminada` });
			} catch {
				toast({
					title: "Error",
					description: "No se pudo eliminar la etiqueta. Puede que esté asignada a algún miembro.",
					variant: "destructive",
				});
			}
		});
